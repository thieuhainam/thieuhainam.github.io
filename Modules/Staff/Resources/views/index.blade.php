...
{{--            <td>{{$row->updated_at}}</td>--}}
{{--            <td style="color: green">{{ ($row->is_active == 1) ? 'Active' : 'Nonactive' }}</td>--}}
{{--            <td>  <a class="btn btn-info" href="{!! route('editstaff',['id'=> $row->staff_id] ) !!}">Edit</a>  @can('staff-delete')|--}}
{{--                {!! Form::open(['method' => 'DELETE','route' => ['destroystaff', $row->staff_id,'0'],'style'=>'display:inline']) !!}--}}
{{--                {!! Form::submit('Delete', ['class' => 'btn btn-danger']) !!}--}}
...
{{--    <a class="btn btn-primary" href="{{route()}}">export</a>--}}
{{--</div>--}}
